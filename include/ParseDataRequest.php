...
class ParseDataRequest
{
     ...
     * @return array|null
     */
    public function parseDataRequest(?string $content): ?array
    {
        if (getallheaders()['Content-Type'] === "application/x-www-form-urlencoded") {
            $data = self::parseApplicationContent($content);
        ...
        if (str_contains(getallheaders()['Content-Type'], "multipart/form-data")) {
            ...
            $data = $this->parseMultipartContent($content, $boundary);
            return empty($data) ? null : $data;
        }
    }
     ...
     */
    public function parseMultipartContent(?string $content, ?string $boundary): ?array
    {
     ...
     * @return array|null
     */
    public static function parseApplicationContent(?string $content): ?array
    {
        $parts = [];
        $keyArray = [];
        $valueArray = [];
